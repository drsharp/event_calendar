module EventCalendar
  ...
  module ClassMethods
    ...
    def has_event_calendar(options={})
      ...
      alias_attribute :name,     display_name_field unless display_name_field == 'name'
    ...
    def events_for_date_range(start_d, end_d, find_options = {})
      self.where(find_options).
        where("(? <= #{self.quoted_table_name}.#{self.end_at_field}) AND (#{self.quoted_table_name}.#{self.start_at_field}< ?)",
          start_d.to_time.utc, end_d.to_time.utc).
